import { ExpansionSet, Rarity, type Card } from "@/model/Card";

const list: Array<Card> = [
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 001",
    name: "Bulbasaur",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 002",
    name: "Ivysaur",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 003",
    name: "Venusaur",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 004",
    name: "Venusaur ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 005",
    name: "Caterpie",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 006",
    name: "Metapod",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 007",
    name: "Butterfree",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 008",
    name: "Weedle",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 009",
    name: "Kakuna",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 010",
    name: "Beedrill",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 011",
    name: "Oddish",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 012",
    name: "Gloom",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 013",
    name: "Vileplume",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 014",
    name: "Paras",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 015",
    name: "Parasect",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 016",
    name: "Venonat",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 017",
    name: "Venomoth",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 018",
    name: "Bellsprout",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 019",
    name: "Weepinbell",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 020",
    name: "Victreebel",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 021",
    name: "Exeggcute",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 022",
    name: "Exeggutor",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 023",
    name: "Exeggutor ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 024",
    name: "Tangela",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 025",
    name: "Scyther",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 026",
    name: "Pinsir",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 027",
    name: "Cottonee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 028",
    name: "Whimsicott",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 029",
    name: "Petilil",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 030",
    name: "Lilligant",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 031",
    name: "Skiddo",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 032",
    name: "Gogoat",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 033",
    name: "Charmander",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 034",
    name: "Charmeleon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 035",
    name: "Charizard",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 036",
    name: "Charizard ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 037",
    name: "Vulpix",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 038",
    name: "Ninetales",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 039",
    name: "Growlithe",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 040",
    name: "Arcanine",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 041",
    name: "Arcanine ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 042",
    name: "Ponyta",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 043",
    name: "Rapidash",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 044",
    name: "Magmar",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 045",
    name: "Flareon",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 046",
    name: "Moltres",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 047",
    name: "Moltres ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 048",
    name: "Heatmor",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 049",
    name: "Salandit",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 050",
    name: "Salazzle",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 051",
    name: "Sizzlipede",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 052",
    name: "Centiskorch",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 053",
    name: "Squirtle",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 054",
    name: "Wartortle",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 055",
    name: "Blastoise",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 056",
    name: "Blastoise ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 057",
    name: "Psyduck",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 058",
    name: "Golduck",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 059",
    name: "Poliwag",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 060",
    name: "Poliwhirl",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 061",
    name: "Poliwrath",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 062",
    name: "Tentacool",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 063",
    name: "Tentacruel",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 064",
    name: "Seel",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 065",
    name: "Dewgong",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 066",
    name: "Shellder",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 067",
    name: "Cloyster",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 068",
    name: "Krabby",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 069",
    name: "Kingler",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 070",
    name: "Horsea",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 071",
    name: "Seadra",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 072",
    name: "Goldeen",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 073",
    name: "Seaking",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 074",
    name: "Staryu",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 075",
    name: "Starmie",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 076",
    name: "Starmie ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 077",
    name: "Magikarp",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 078",
    name: "Gyarados",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 079",
    name: "Lapras",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 080",
    name: "Vaporeon",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 081",
    name: "Omanyte",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 082",
    name: "Omastar",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 083",
    name: "Articuno",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 084",
    name: "Articuno ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 085",
    name: "Ducklett",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 086",
    name: "Swanna",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 087",
    name: "Froakie",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 088",
    name: "Frogadier",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 089",
    name: "Greninja",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 090",
    name: "Pyukumuku",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 091",
    name: "Bruxish",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 092",
    name: "Snom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 093",
    name: "Frosmoth",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 094",
    name: "Pikachu",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 095",
    name: "Raichu",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 096",
    name: "Pikachu ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 097",
    name: "Magnemite",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 098",
    name: "Magneton",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 099",
    name: "Voltorb",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 100",
    name: "Electrode",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 101",
    name: "Electabuzz",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 102",
    name: "Jolteon",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 103",
    name: "Zapdos",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 104",
    name: "Zapdos ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 105",
    name: "Blitzle",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 106",
    name: "Zebstrika",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 107",
    name: "Tynamo",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 108",
    name: "Eelektrik",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 109",
    name: "Eelektross",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 110",
    name: "Helioptile",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 111",
    name: "Heliolisk",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 112",
    name: "Pincurchin",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 113",
    name: "Clefairy",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 114",
    name: "Clefable",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 115",
    name: "Abra",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 116",
    name: "Kadabra",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 117",
    name: "Alakazam",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 118",
    name: "Slowpoke",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 119",
    name: "Slowbro",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 120",
    name: "Gastly",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 121",
    name: "Haunter",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 122",
    name: "Gengar",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 123",
    name: "Gengar ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 124",
    name: "Drowzee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 125",
    name: "Hypno",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 126",
    name: "Mr. Mime",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 127",
    name: "Jynx",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 128",
    name: "Mewtwo",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 129",
    name: "Mewtwo ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 130",
    name: "Ralts",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 131",
    name: "Kirlia",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 132",
    name: "Gardevoir",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 133",
    name: "Woobat",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 134",
    name: "Swoobat",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 135",
    name: "Golett",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 136",
    name: "Golurk",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 137",
    name: "Sandshrew",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 138",
    name: "Sandslash",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 139",
    name: "Diglett",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 140",
    name: "Dugtrio",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 141",
    name: "Mankey",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 142",
    name: "Primeape",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 143",
    name: "Machop",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 144",
    name: "Machoke",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 145",
    name: "Machamp",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 146",
    name: "Machamp ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 147",
    name: "Geodude",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 148",
    name: "Graveler",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 149",
    name: "Golem",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 150",
    name: "Onix",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 151",
    name: "Cubone",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 152",
    name: "Marowak",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 153",
    name: "Marowak ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 154",
    name: "Hitmonlee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 155",
    name: "Hitmonchan",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 156",
    name: "Rhyhorn",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 157",
    name: "Rhydon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 158",
    name: "Kabuto",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 159",
    name: "Kabutops",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 160",
    name: "Mienfoo",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 161",
    name: "Mienshao",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 162",
    name: "Clobbopus",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 163",
    name: "Grapploct",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 164",
    name: "Ekans",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 165",
    name: "Arbok",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 166",
    name: "Nidoran F",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 167",
    name: "Nidorina",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 168",
    name: "Nidoqueen",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 169",
    name: "Nidoran M",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 170",
    name: "Nidorino",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 171",
    name: "Nidoking",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 172",
    name: "Zubat",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 173",
    name: "Golbat",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 174",
    name: "Grimer",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 175",
    name: "Muk",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 176",
    name: "Koffing",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 177",
    name: "Weezing",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 178",
    name: "Mawile",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 179",
    name: "Pawniard",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 180",
    name: "Bisharp",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 181",
    name: "Meltan",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 182",
    name: "Melmetal",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 183",
    name: "Dratini",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 184",
    name: "Dragonair",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 185",
    name: "Dragonite",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 186",
    name: "Pidgey",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 187",
    name: "Pidgeotto",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 188",
    name: "Pidgeot",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 189",
    name: "Rattata",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 190",
    name: "Raticate",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 191",
    name: "Spearow",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 192",
    name: "Fearow",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 193",
    name: "Jigglypuff",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 194",
    name: "Wigglytuff",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 195",
    name: "Wigglytuff ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 196",
    name: "Meowth",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 197",
    name: "Persian",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 198",
    name: "Farfetch'd",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 199",
    name: "Doduo",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 200",
    name: "Dodrio",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 201",
    name: "Lickitung",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 202",
    name: "Chansey",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 203",
    name: "Kangaskhan",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 204",
    name: "Tauros",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 205",
    name: "Ditto",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 206",
    name: "Eevee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 207",
    name: "Eevee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 208",
    name: "Eevee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 209",
    name: "Porygon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 210",
    name: "Aerodactyl",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 211",
    name: "Snorlax",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 212",
    name: "Minccino",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 213",
    name: "Cinccino",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 214",
    name: "Wooloo",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 215",
    name: "Dubwool",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 216",
    name: "Helix Fossil",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 217",
    name: "Dome Fossil",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 218",
    name: "Old Amber",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 219",
    name: "Erika",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 220",
    name: "Misty",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 221",
    name: "Blaine",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 222",
    name: "Koga",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 223",
    name: "Giovanni",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 224",
    name: "Brock",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 225",
    name: "Sabrina",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 226",
    name: "Lt. Surge",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 227",
    name: "Bulbasaur",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 228",
    name: "Gloom",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 229",
    name: "Pinsir",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 230",
    name: "Charmander",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 231",
    name: "Rapidash",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 232",
    name: "Squirtle",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 233",
    name: "Gyarados",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 234",
    name: "Lapras",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 235",
    name: "Electrode",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 236",
    name: "Alakazam",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 237",
    name: "Slowpoke",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 238",
    name: "Diglett",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 239",
    name: "Cubone",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 240",
    name: "Nidoqueen",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 241",
    name: "Nidoking",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 242",
    name: "Golbat",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 243",
    name: "Weezing",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 244",
    name: "Dragonite",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 245",
    name: "Pidgeot",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 246",
    name: "Meowth",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 247",
    name: "Ditto",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 248",
    name: "Eevee",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 249",
    name: "Porygon",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 250",
    name: "Snorlax",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 251",
    name: "Venusaur ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 252",
    name: "Exeggutor ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 253",
    name: "Charizard ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 254",
    name: "Arcanine ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 255",
    name: "Moltres ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 256",
    name: "Blastoise ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 257",
    name: "Starmie ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 258",
    name: "Articuno ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 259",
    name: "Pikachu ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 260",
    name: "Zapdos ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 261",
    name: "Gengar ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 262",
    name: "Mewtwo ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 263",
    name: "Machamp ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 264",
    name: "Marowak ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 265",
    name: "Wigglytuff ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 266",
    name: "Erika",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 267",
    name: "Misty",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 268",
    name: "Blaine",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 269",
    name: "Koga",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 270",
    name: "Giovanni",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 271",
    name: "Brock",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 272",
    name: "Sabrina",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 273",
    name: "Lt. Surge",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 274",
    name: "Moltres ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 275",
    name: "Articuno ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 276",
    name: "Zapdos ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 277",
    name: "Gengar ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 278",
    name: "Machamp ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 279",
    name: "Wigglytuff ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 280",
    name: "Charizard ex",
    rarity: Rarity.ThreeStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 281",
    name: "Pikachu ex",
    rarity: Rarity.ThreeStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 282",
    name: "Mewtwo ex",
    rarity: Rarity.ThreeStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 283",
    name: "Mew",
    rarity: Rarity.ThreeStars,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 284",
    name: "Charizard ex",
    rarity: Rarity.Crown,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 285",
    name: "Pikachu ex",
    rarity: Rarity.Crown,
  },
  {
    expansion: ExpansionSet.GeneticApex,
    number: "A1 286",
    name: "Mewtwo ex",
    rarity: Rarity.Crown,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 001",
    name: "Exeggcute",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 002",
    name: "Exeggutor",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 003",
    name: "Celebi ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 004",
    name: "Snivy",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 005",
    name: "Servine",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 006",
    name: "Serperior",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 007",
    name: "Morelull",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 008",
    name: "Shiinotic",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 009",
    name: "Dhelmise",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 010",
    name: "Ponyta",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 011",
    name: "Rapidash",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 012",
    name: "Magmar",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 013",
    name: "Larvesta",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 014",
    name: "Volcarona",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 015",
    name: "Salandit",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 016",
    name: "Salazzle",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 017",
    name: "Magikarp",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 018",
    name: "Gyarados ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 019",
    name: "Vaporeon",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 020",
    name: "Finneon",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 021",
    name: "Lumineon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 022",
    name: "Chewtle",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 023",
    name: "Drednaw",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 024",
    name: "Cramorant",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 025",
    name: "Pikachu",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 026",
    name: "Raichu",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 027",
    name: "Electabuzz",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 028",
    name: "Joltik",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 029",
    name: "Galvantula",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 030",
    name: "Dedenne",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 031",
    name: "Mew",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 032",
    name: "Mew ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 033",
    name: "Sigilyph",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 034",
    name: "Elgyem",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 035",
    name: "Beheeyem",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 036",
    name: "Flabebe",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 037",
    name: "Floette",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 038",
    name: "Florges",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 039",
    name: "Swirlix",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 040",
    name: "Slurpuff",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 041",
    name: "Mankey",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 042",
    name: "Primeape",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 043",
    name: "Geodude",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 044",
    name: "Graveler",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 045",
    name: "Golem",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 046",
    name: "Aerodactyl ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 047",
    name: "Marshadow",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 048",
    name: "Stonjourner",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 049",
    name: "Koffing",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 050",
    name: "Weezing",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 051",
    name: "Purrloin",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 052",
    name: "Liepard",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 053",
    name: "Venipede",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 054",
    name: "Whirlipede",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 055",
    name: "Scolipede",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 056",
    name: "Druddigon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 057",
    name: "Pidgey",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 058",
    name: "Pidgeotto",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 059",
    name: "Pidgeot ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 060",
    name: "Tauros",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 061",
    name: "Eevee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 062",
    name: "Chatot",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 063",
    name: "Old Amber",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 064",
    name: "Pokemon Flute",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 065",
    name: "Mythical Slab",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 066",
    name: "Budding Expeditioner",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 067",
    name: "Blue",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 068",
    name: "Leaf",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 069",
    name: "Exeggutor",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 070",
    name: "Serperior",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 071",
    name: "Salandit",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 072",
    name: "Vaporeon",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 073",
    name: "Dedenne",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 074",
    name: "Marshadow",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 075",
    name: "Celebi ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 076",
    name: "Gyarados ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 077",
    name: "Mew ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 078",
    name: "Aerodactyl ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 079",
    name: "Pidgeot ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 080",
    name: "Budding Expeditioner",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 081",
    name: "Blue",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 082",
    name: "Leaf",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 083",
    name: "Mew ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 084",
    name: "Aerodactyl ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 085",
    name: "Celebi ex",
    rarity: Rarity.ThreeStars,
  },
  {
    expansion: ExpansionSet.MythicalIsland,
    number: "A1a 086",
    name: "Mew ex",
    rarity: Rarity.Crown,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 001",
    name: "Oddish",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 002",
    name: "Gloom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 003",
    name: "Bellossom",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 004",
    name: "Tangela",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 005",
    name: "Tangrowth",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 006",
    name: "Yanma",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 007",
    name: "Yanmega ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 008",
    name: "Roselia",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 009",
    name: "Roserade",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 010",
    name: "Turtwig",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 011",
    name: "Grotle",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 012",
    name: "Torterra",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 013",
    name: "Kricketot",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 014",
    name: "Kricketune",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 015",
    name: "Burmy",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 016",
    name: "Wormadam",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 017",
    name: "Combee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 018",
    name: "Vespiquen",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 019",
    name: "Carnivine",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 020",
    name: "Leafeon",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 021",
    name: "Mow Rotom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 022",
    name: "Shaymin",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 023",
    name: "Magmar",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 024",
    name: "Magmortar",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 025",
    name: "Slugma",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 026",
    name: "Magcargo",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 027",
    name: "Chimchar",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 028",
    name: "Monferno",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 029",
    name: "Infernape ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 030",
    name: "Heat Rotom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 031",
    name: "Swinub",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 032",
    name: "Piloswine",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 033",
    name: "Mamoswine",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 034",
    name: "Regice",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 035",
    name: "Piplup",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 036",
    name: "Prinplup",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 037",
    name: "Empoleon",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 038",
    name: "Buizel",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 039",
    name: "Floatzel",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 040",
    name: "Shellos",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 041",
    name: "Gastrodon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 042",
    name: "Finneon",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 043",
    name: "Lumineon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 044",
    name: "Snover",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 045",
    name: "Abomasnow",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 046",
    name: "Glaceon",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 047",
    name: "Wash Rotom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 048",
    name: "Frost Rotom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 049",
    name: "Palkia ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 050",
    name: "Manaphy",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 051",
    name: "Magnemite",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 052",
    name: "Magneton",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 053",
    name: "Magnezone",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 054",
    name: "Voltorb",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 055",
    name: "Electrode",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 056",
    name: "Electabuzz",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 057",
    name: "Electivire",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 058",
    name: "Shinx",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 059",
    name: "Luxio",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 060",
    name: "Luxray",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 061",
    name: "Pachirisu ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 062",
    name: "Rotom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 063",
    name: "Togepi",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 064",
    name: "Togetic",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 065",
    name: "Togekiss",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 066",
    name: "Misdreavus",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 067",
    name: "Mismagius ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 068",
    name: "Ralts",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 069",
    name: "Kirlia",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 070",
    name: "Duskull",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 071",
    name: "Dusclops",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 072",
    name: "Dusknoir",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 073",
    name: "Drifloon",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 074",
    name: "Drifblim",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 075",
    name: "Uxie",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 076",
    name: "Mesprit",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 077",
    name: "Azelf",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 078",
    name: "Giratina",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 079",
    name: "Cresselia",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 080",
    name: "Rhyhorn",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 081",
    name: "Rhydon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 082",
    name: "Rhyperior",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 083",
    name: "Gligar",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 084",
    name: "Gliscor",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 085",
    name: "Hitmontop",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 086",
    name: "Nosepass",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 087",
    name: "Regirock",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 088",
    name: "Cranidos",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 089",
    name: "Rampardos",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 090",
    name: "Wormadam",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 091",
    name: "Riolu",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 092",
    name: "Lucario",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 093",
    name: "Hippopotas",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 094",
    name: "Hippowdon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 095",
    name: "Gallade ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 096",
    name: "Murkrow",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 097",
    name: "Honchkrow",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 098",
    name: "Sneasel",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 099",
    name: "Weavile ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 100",
    name: "Poochyena",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 101",
    name: "Mightyena",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 102",
    name: "Stunky",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 103",
    name: "Skuntank",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 104",
    name: "Spiritomb",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 105",
    name: "Skorupi",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 106",
    name: "Drapion",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 107",
    name: "Croagunk",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 108",
    name: "Toxicroak",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 109",
    name: "Darkrai",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 110",
    name: "Darkrai ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 111",
    name: "Skarmory",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 112",
    name: "Registeel",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 113",
    name: "Shieldon",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 114",
    name: "Bastiodon",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 115",
    name: "Wormadam",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 116",
    name: "Bronzor",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 117",
    name: "Bronzong",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 118",
    name: "Probopass",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 119",
    name: "Dialga ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 120",
    name: "Heatran",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 121",
    name: "Gible",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 122",
    name: "Gabite",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 123",
    name: "Garchomp",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 124",
    name: "Lickitung",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 125",
    name: "Lickilicky ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 126",
    name: "Eevee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 127",
    name: "Porygon",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 128",
    name: "Porygon2",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 129",
    name: "Porygon-Z",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 130",
    name: "Aipom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 131",
    name: "Ambipom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 132",
    name: "Starly",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 133",
    name: "Staravia",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 134",
    name: "Staraptor",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 135",
    name: "Bidoof",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 136",
    name: "Bibarel",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 137",
    name: "Buneary",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 138",
    name: "Lopunny",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 139",
    name: "Glameow",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 140",
    name: "Purugly",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 141",
    name: "Chatot",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 142",
    name: "Fan Rotom",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 143",
    name: "Regigigas",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 144",
    name: "Skull Fossil",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 145",
    name: "Armor Fossil",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 146",
    name: "Pokemon Communication",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 147",
    name: "Giant Cape",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 148",
    name: "Rocky Helmet",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 149",
    name: "Lum Berry",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 150",
    name: "Cyrus",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 151",
    name: "Team Galactic Grunt",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 152",
    name: "Cynthia",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 153",
    name: "Volkner",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 154",
    name: "Dawn",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 155",
    name: "Mars",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 156",
    name: "Tangrowth",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 157",
    name: "Combee",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 158",
    name: "Carnivine",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 159",
    name: "Shaymin",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 160",
    name: "Mamoswine",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 161",
    name: "Gastrodon",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 162",
    name: "Manaphy",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 163",
    name: "Shinx",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 164",
    name: "Rotom",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 165",
    name: "Drifloon",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 166",
    name: "Mesprit",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 167",
    name: "Giratina",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 168",
    name: "Cresselia",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 169",
    name: "Rhyperior",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 170",
    name: "Lucario",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 171",
    name: "Hippopotas",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 172",
    name: "Spiritomb",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 173",
    name: "Croagunk",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 174",
    name: "Heatran",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 175",
    name: "Garchomp",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 176",
    name: "Staraptor",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 177",
    name: "Bidoof",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 178",
    name: "Glameow",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 179",
    name: "Regigigas",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 180",
    name: "Yanmega ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 181",
    name: "Infernape ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 182",
    name: "Palkia ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 183",
    name: "Pachirisu ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 184",
    name: "Mismagius ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 185",
    name: "Gallade ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 186",
    name: "Weavile ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 187",
    name: "Darkrai ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 188",
    name: "Dialga ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 189",
    name: "Lickilicky ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 190",
    name: "Cyrus",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 191",
    name: "Team Galactic Grunt",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 192",
    name: "Cynthia",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 193",
    name: "Volkner",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 194",
    name: "Dawn",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 195",
    name: "Mars",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 196",
    name: "Yanmega ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 197",
    name: "Infernape ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 198",
    name: "Pachirisu ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 199",
    name: "Mismagius ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 200",
    name: "Gallade ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 201",
    name: "Weavile ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 202",
    name: "Darkrai ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 203",
    name: "Lickilicky ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 204",
    name: "Palkia ex",
    rarity: Rarity.ThreeStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 205",
    name: "Dialga ex",
    rarity: Rarity.ThreeStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 206",
    name: "Palkia ex",
    rarity: Rarity.Crown,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2 207",
    name: "Dialga ex",
    rarity: Rarity.Crown,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 001",
    name: "Heracross",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 002",
    name: "Burmy",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 003",
    name: "Mothim",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 004",
    name: "Combee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 005",
    name: "Vespiquen",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 006",
    name: "Cherubi",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 007",
    name: "Cherrim",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 008",
    name: "Cherrim",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 009",
    name: "Carnivine",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 010",
    name: "Leafeon ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 011",
    name: "Houndour",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 012",
    name: "Houndoom",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 013",
    name: "Heatran",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 014",
    name: "Marill",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 015",
    name: "Azumarill",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 016",
    name: "Barboach",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 017",
    name: "Whiscash",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 018",
    name: "Snorunt",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 019",
    name: "Froslass",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 020",
    name: "Snover",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 021",
    name: "Abomasnow",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 022",
    name: "Glaceon ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 023",
    name: "Origin Forme Palkia",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 024",
    name: "Phione",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 025",
    name: "Pikachu",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 026",
    name: "Raichu",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 027",
    name: "Electrike",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 028",
    name: "Manectric",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 029",
    name: "Clefairy",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 030",
    name: "Clefable",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 031",
    name: "Gastly",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 032",
    name: "Haunter",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 033",
    name: "Gengar",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 034",
    name: "Unown",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 035",
    name: "Rotom",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 036",
    name: "Sudowoodo",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 037",
    name: "Phanpy",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 038",
    name: "Donphan",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 039",
    name: "Larvitar",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 040",
    name: "Pupitar",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 041",
    name: "Tyranitar",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 042",
    name: "Nosepass",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 043",
    name: "Meditite",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 044",
    name: "Medicham",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 045",
    name: "Gible",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 046",
    name: "Gabite",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 047",
    name: "Garchomp ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 048",
    name: "Zubat",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 049",
    name: "Golbat",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 050",
    name: "Crobat",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 051",
    name: "Croagunk",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 052",
    name: "Toxicroak",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 053",
    name: "Magnemite",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 054",
    name: "Magneton",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 055",
    name: "Magnezone",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 056",
    name: "Mawile",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 057",
    name: "Probopass ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 058",
    name: "Bronzor",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 059",
    name: "Bronzong",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 060",
    name: "Origin Forme Dialga",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 061",
    name: "Giratina",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 062",
    name: "Eevee",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 063",
    name: "Snorlax",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 064",
    name: "Hoothoot",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 065",
    name: "Noctowl",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 066",
    name: "Starly",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 067",
    name: "Staravia",
    rarity: Rarity.OneDiamond,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 068",
    name: "Staraptor",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 069",
    name: "Shaymin",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 070",
    name: "Arceus",
    rarity: Rarity.ThreeDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 071",
    name: "Arceus ex",
    rarity: Rarity.FourDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 072",
    name: "Irida",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 073",
    name: "Celestic Town Elder",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 074",
    name: "Barry",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 075",
    name: "Adaman",
    rarity: Rarity.TwoDiamonds,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 076",
    name: "Houndoom",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 077",
    name: "Marill",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 078",
    name: "Unown",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 079",
    name: "Sudowoodo",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 080",
    name: "Magnemite",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 081",
    name: "Shaymin",
    rarity: Rarity.OneStar,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 082",
    name: "Leafeon ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 083",
    name: "Glaceon ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 084",
    name: "Garchomp ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 085",
    name: "Probopass ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 086",
    name: "Arceus ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 087",
    name: "Irida",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 088",
    name: "Celestic Town Elder",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 089",
    name: "Barry",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 090",
    name: "Adaman",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 091",
    name: "Leafeon ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 092",
    name: "Glaceon ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 093",
    name: "Garchomp ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 094",
    name: "Probopass ex",
    rarity: Rarity.TwoStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 095",
    name: "Arceus ex",
    rarity: Rarity.ThreeStars,
  },
  {
    expansion: ExpansionSet.SpaceTimeSmackdown,
    number: "A2a 096",
    name: "Arceus ex",
    rarity: Rarity.Crown,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 001",
    name: "Potion",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 002",
    name: "X Speed",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 003",
    name: "Hand Scope",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 004",
    name: "Pokedex",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 005",
    name: "Poke Ball",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 006",
    name: "Red Card",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 007",
    name: "Professor's Research",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 008",
    name: "Pokedex",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 009",
    name: "Pikachu",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 010",
    name: "Mewtwo",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 011",
    name: "Chansey",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 012",
    name: "Meowth",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 013",
    name: "Butterfree",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 014",
    name: "Lapras ex",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 015",
    name: "Pikachu",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 016",
    name: "Clefairy",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 017",
    name: "Mankey",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 018",
    name: "Venusaur",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 019",
    name: "Greninja",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 020",
    name: "Haunter",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 021",
    name: "Onix",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 022",
    name: "Jigglypuff",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 023",
    name: "Bulbasaur",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 024",
    name: "Magnemite",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 025",
    name: "Moltres ex",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 026",
    name: "Pikachu",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 027",
    name: "Snivy",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 028",
    name: "Volcarona",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 029",
    name: "Blastoise",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 030",
    name: "Eevee",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 031",
    name: "Cinccino",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 032",
    name: "Charmander",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 033",
    name: "Squirtle",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 034",
    name: "Piplup",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 035",
    name: "Turtwig",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 036",
    name: "Electivire",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 037",
    name: "Cresselia ex",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 038",
    name: "Misdreavus",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 039",
    name: "Skarmory",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 040",
    name: "Chimchar",
    rarity: Rarity.Promo,
  },
  {
    expansion: ExpansionSet.PromoA,
    number: "P-A 041",
    name: "Togepi",
    rarity: Rarity.Promo,
  },
];

export function fromNumber(number:string): Card {
  return list.find(x => x.number == number) || {
    name: "Missigno",
    number: "??",
    expansion: ExpansionSet.PromoA,
    rarity: Rarity.Crown
  }
}

export default list;
